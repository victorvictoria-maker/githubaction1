...
// Graphics buffer to store the drawings
let graphicsBuffer;
...
function draw() {
  ...
  if (mouseIsPressed) {
    if (isEraser) {
      ...
      graphicsBuffer.ellipse(mouseX, mouseY, eraserSize);
    } else {
      // Draw with the selected brush size and color
      graphicsBuffer.strokeWeight(brushSize);
      graphicsBuffer.stroke(brushColor);
      graphicsBuffer.line(mouseX, mouseY, pmouseX, pmouseY);
    }
  }

  // Display the graphics buffer on the canvas
